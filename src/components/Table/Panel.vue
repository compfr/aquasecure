<template>
    <div>
        <b-row>
            <b-col cols="12">
                <vue-good-table v-if="(noPagination && model) || (!noPagination && model.data)"
                    :columns="columns"
                    :rows="noPagination ? model : model.data"
                    :rtl="direction"
                    :pagination-options="{
                        enabled: !noPagination,
                        perPage: panelPerPage
                    }"
                    :sort-options="{
                        enabled: sortEnabled,
                    }"
                ...
                >
                    ...
                    <template
                        ...
                        slot-scope="props"
                    >
                        <div v-if="showBottom" class="d-flex justify-content-between flex-wrap">
                            <div class="d-flex align-items-center mb-0 mt-1">
                                <span class="text-nowrap">
                                  Showing 1 to
                                </span>
                                <b-form-select
                                    v-model="panelPageLength"
                                    :options="['2','25','50','100']"
                                    class="mx-1"
                                    @input="reload"
                                />
...
<script>
    ...
    export default {
        ...
        created() {
            if (this.initialGet) {
                this.panelPageLength = this.pageLength;
                this.reload()
            }
        },
        methods: {
            reload(){
                this.additionalParams['per_page'] = this.panelPageLength;
                get(this.urlApi,this.additionalParams).then(res => {
                    this.model = res.data.data;
                    this.panelPageLength = this.noPagination ? res.data.data.length : res.data.data.per_page
                });
            },
            update(){
                var url = '';
                for(var i=0;i<this.model.links.length;i++){
                    if(this.model.links[i].active){
                        url = this.model.links[i].url;
                        break;
                    }
                }
            ...
            nextPage() {
                get(this.model.next_page_url).then((res) => {
                    ...
                    this.panelPageLength = res.data.data.per_page
                })
            },
            prevPage() {
