            ...
            <b-row>
                ...
                <b-col md="12" class="mb-2">
                    ...
                    <template v-if="model.domains.length > 0">
                        <b-badge class="mr-1" v-for="(item, index) in  model.domains" :key="`domain-${item.id}`">
                            {{item.domain.title}}
                        </b-badge>
                    </template>
                    <template v-else>
                ...
                <b-col md="12" class="mb-2">
                    ...
                    <template v-if="model.sub_controls.length > 0">
                        <b-badge class="mr-1" v-for="(item, index) in  model.sub_controls"
                                 :key="`sub_control-${item.id}`">
...
<script>
    ...
    export default {
        ...
        created() {
            get(`/api/controls/${this.$route.params.id}`).then((res) => {
