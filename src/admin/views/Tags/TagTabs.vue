    ...
    <div v-if="show">
        <b-tabs v-model="selectedTab" @input="updateRoute">
            <template v-for="(item, index) in tag_types">
                <b-tab :title="item.title" :key="`tag-type-index-${index}`">
...
<script>
    ...
    import GRCTags from "@/admin/views/Tags/GRCTags";
            ...
            getTagTypes(){
                ...
                get('/api/tag_types').then((res)=>{
                    ...
                    if (this.$route.query && this.$route.query.tab) {
                        // if (this.$route.query.tab == 'evidence') {
                        //     this.selectedTab = 0;
                        // }
                        // for(var i=0;i<=this.tag_types.length;i++){
                        //     if (this.$route.query.tab === this.tag_types[i-1]['slug']) {
                        //         this.selectedTab = i;
                        //         break;
                        //     }
                        // }
